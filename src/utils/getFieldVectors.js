  ...
  for (const obj of chargedObjects) {
    if (!(obj.type === 'surface'))
      ...
      let gridVector3 = []
        ...
        case 'cylinder': {
          // treat wire as a finite cylinder: sample uniformly over lateral
          // surface and caps (area-proportional)
          ...
          // lateral surface: make a rows x cols grid over (height x
          // circumference)
          if (lateralCount > 0) {
            const cols = Math.max(
                1,
                Math.ceil(Math.sqrt(
                    lateralCount * (2 * Math.PI * radius) /
            ...
            for (let ri = 0; ri < rows; ri++) {
              for (let ci = 0; ci < cols; ci++) {
                if (gridVector3.length >= lateralCount)
                  ...
                  gridVector3.push(
                      new THREE.Vector3(x, y, z).add(objPos.clone()))
              }
            }
          }

          // caps: simple deterministic grid in bounding square, reject outside
          // circle
                ...
                for (let c = 0; c < cols; c++) {
                  ...
                  const x = -radius + (c + 0.5) * (2 * radius / cols)
                    ...
                    if (x * x + z * z <= radius * radius) {
                      gridVector3.push(
                          new THREE.Vector3(x, capY, z).add(objPos.clone()))
                    }
                }
              }
          }
        ...
        case 'cuboid': {
          ...
          const rem = exact.map((v, i) => ({frac: v - counts[i], i}))
