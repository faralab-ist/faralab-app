...
export default function getFieldVector3(objects, gridSize = 10, step = 1, showOnlyPlane = false, showOnlyGaussianField = false, min) {
  const fieldVectors = []
    ...
    for (let x = -gridSize; x <= gridSize; x += step) {
      for (let y = -yLevel; y <= yLevel; y += step) {
        for (let z = -gridSize; z <= gridSize; z += step) {
          ...
          if (Math.abs(field.lengthSq()) > min) {
  ...
  for (const obj of objects) {
    ...
    switch (obj.surfaceType) {
      case 'sphere': {
        ...
        samples = fibonacciSpherePoints(sampleCount, obj.radius)
        samples = samples.map(p => p.add(objPos.clone().multiplyScalar(2)))
        break
      }
        ...
        if (lateralCount > 0) {
          ...
          for (let ri = 0; ri < rows; ri++) {
            for (let ci = 0; ci < cols; ci++) {
              ...
              samples.push(new THREE.Vector3(x, y, z).add(objPos.clone().multiplyScalar(2)))
            }
          }
        }
          ...
          for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
              ...
              if (x * x + z * z <= radius * radius) {
                samples.push(new THREE.Vector3(x, capY, z).add(objPos.clone().multiplyScalar(2)))
              }
            }
          }
        }
        ...
        for (let fi = 0; fi < faces.length; fi++) {
          ...
          for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
              ...
              p[f.v] = vv
              p[f.wAxis] = f.wPos
              samples.push(p.add(objPos.clone().multiplyScalar(2)))
            }
          }
        }
        break
