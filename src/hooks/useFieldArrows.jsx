...
export default function FieldArrows({ objects, showOnlyPlane = false, showOnlyGaussianField = false, fieldThreshold = 0.1, gridSize = 10, step = 1, min = .5 }) {
    
    const vectors = useMemo(
        () => getFieldVector3(objects, gridSize, step, showOnlyPlane, showOnlyGaussianField, min),
    ...
    for (const { position, field } of vectors) {
        ...
        const normalized = Math.min(Math.max(logMag, 0), 1);
